...
Endpoints:
- GET    /rfqs/{rfqId}/stages/{stageId}/files  — #28 List files for stage
- GET    /files/{fileId}/download               — #29 Download file (stream or signed URL)
- DELETE /files/{fileId}                        — #30 Delete file (soft delete)
"""
...
# ── Nested router for #28 (list files by stage) ──────
...
def list_stage_files(
    ...
    """#28 — List files for a stage."""
    return ctrl.list_for_stage(rfq_id, stage_id)


# ── Flat router for #29-#30 (download/delete by file ID) ─
...
def download_file(
    ...
    """#29 — Download file by ID. Returns file stream."""
...
def delete_file(
    file_id: UUID,
    _auth=Depends(require_permission(Permissions.FILE_DELETE)),
    ctrl: FileController = Depends(get_file_controller),
):
    """#30 — Soft delete file."""
    ctrl.delete(file_id)
