...
Last updated: 2026-03-19 (LG-04 implemented)
...
| LG-03 | Skipped stages deflate RFQ progress | RESOLVED | Updated progress calculations to exclude `Skipped` stages in both RFQ stage-advance and RFQ terminal transition flows. Progress now reflects non-skipped work and reaches 100 when remaining stages are skipped after completed work. | src/controllers/rfq_stage_controller.py; src/controllers/rfq_controller.py; tests/unit/test_rfq_stage_controller.py; tests/unit/test_rfq_controller.py | `pytest tests/unit/test_rfq_controller.py tests/unit/test_rfq_stage_controller.py -q` → 20 passed. Added tests for skipped-stage exclusion in both controllers. | - | Implemented on branch `fix/lg-03-lg-04-progress-and-create-status`; not yet merged to `main`. |
| LG-04 | RFQ create relies on model default status | RESOLVED | RFQ create flow now explicitly sets `rfq_data["status"] = "In preparation"` before datasource create call, removing hidden dependency on model default. | src/controllers/rfq_controller.py; tests/unit/test_rfq_controller.py | Same focused test run (20 passed). Added create-flow test asserting explicit status is passed to datasource. | - | Implemented on branch `fix/lg-03-lg-04-progress-and-create-status`; not yet merged to `main`. |
